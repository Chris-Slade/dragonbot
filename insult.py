...
def random_insult(insults_file='insults.json'):
    ...
    These are loaded from a JSON file specified by the `insults_file`
    parameter, defaulting to `insults.json`. The file should be given as an
    object with two fields: `encoding`, which optionally specifies the encoding
    of the insults (a parameter to `codecs.decode`), and `insults`, which is an
    array containing the insults.

    Alternatively, if `DRAGONBOT_INSULTS` is set in the environment, that
    environment variable will be parsed as JSON and used instead.
    ...
    if not hasattr(random_insult, '_cache'):
        if constants.INSULTS_ENV_VAR in os.environ:
            obj = json.loads(os.environ[constants.INSULTS_ENV_VAR])
        else:
